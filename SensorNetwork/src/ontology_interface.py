...
def loadOntology(filename=ONTO_FILENAME, path="./"):
    ...
    owlready2.JAVA_EXE = "C:/Program Files/Java/jdk-17.0.5/bin/java.exe"
...
def insertNumberPlate(onto, nr="ABC123"):
...
def insertPerson(onto, persID, name="Kjell Kriminell"):
...
def setCurrentNumberPlate(onto, nr="AAA111"):
...
def get_Human_Aware_Planning_session_modifiers(onto):
    """
...
def getCurrentNumberPlate(onto):
    s=PREFIX+"""
        SELECT ?plate ?number
...
def getNumberPlateThreatLevels(onto, nr="ABC123"):
    s=PREFIX+"""
...
def getCriminalRecordThreatEstimate(onto, nr="ABC123"):
...
def getASPThreatLevels(onto, nr="ABC123"):
    s = PREFIX + """ SELECT ?e WHERE{
        ?record fnord:hasPlateNumber \""""+nr+"""\"
        ?record fnord:hasRegisteredOwner ?person
...
def clearCurrentNumberPlate(onto):
    s=PREFIX+"""
